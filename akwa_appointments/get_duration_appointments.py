...
def main():
    ...
    for client in clients:
        ...
        total_duration = fetch_payment_data(client)
        ...
        print(f"✅ {client['name']} → Total duration: {total_duration}")

    # Сортування за sort_index
    output_data.sort(key=lambda x: x["sort_index"])

    # Запис у CSV
    with open("list_of_akwa_appointment_duration.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Client", "Total Appointment Duration (Last 30 Days)"])
        for row in output_data:
            writer.writerow([row["Client"], row["Total"]])

    print("\n📁 Data written to 'list_of_akwa_appointments_durations.csv'")
